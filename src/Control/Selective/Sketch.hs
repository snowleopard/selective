...
ft1 f = select (Pure (Right 8)) (liftSelect f)
    ===
    select (Pure (Right 8)) (Select (Pure (Left ())) (const <$> f))
    ===
    Select (select (fmap Right <$> Pure (Right 8))
        ((\k a -> bimap (,a) ($a) k) <$> Pure (Left ())))
        (uncurry <$> (const <$> f))
    ===
    Select (select (Pure (Right (Right 8)))
        (Pure (\a -> Left ((), a))))
        (uncurry . const <$> f)
    ===
    Select (Pure (Right 8)) (uncurry . const <$> f)
    ===
    Effect (Pure 8) (void f)

-- Detect two unnecessary effects
ft2 :: Functor f => f (Either a (b -> Int)) -> f (a -> b -> Int) -> Select f Int
ft2 f g = select (Pure (Right 8)) (Select (liftSelect f) g)
    ===
    select (Pure (Right 8)) (Select (Select (Pure (Left ())) (const <$> f)) g)
    ===
    Select (select (fmap Right <$> Pure (Right 8))
        ((\k a -> bimap (,a) ($a) k) <$> (Select (Pure (Left ())) (const <$> f))))
        (uncurry <$> g)
    ===
    Select (select (Pure (Right (Right 8)))
        (Select (Pure (Left ())) ((((\k a -> bimap (,a) ($a) k).) <$> const <$> f))))
        (uncurry <$> g)
    ===
    Select (Select (select (Pure (Right (Right (Right 8))))
                (Pure (\a -> Left ((), a))))
...
ft3 f = select (Pure (Left ())) (Select (Pure (Right (const 8))) f)
    ===
    Select
        (select (fmap Right <$> Pure (Left ()))
    ...
    Select
        ...
        (uncurry <$> f)
    ===
    Select (select (Pure (Left ())) (Pure (const $ Right 8)))
        (uncurry <$> f)
    ===
    Select (either (const $ Right 8) id <$> Pure (Left ()))
        (uncurry <$> f)
    ===
    Select (Pure (Right 8)) (uncurry <$> f)
    ===
    Effect (Pure 8) (void f)
...
ft4 f g = select (liftSelect f) (Select (Pure (Right (const 8))) g)
    ===
    select (Select (Pure (Left ())) (const <$> f)) (Select (Pure (Right (const 8))) g)
    ===
    Select (select (fmap Right <$> Select (Pure (Left ())) (const <$> f))
        ((\k a -> bimap (,a) ($a) k) <$> Pure (Right (const 8))))
        (uncurry <$> g)
    ===
    Select (select (Select (Pure (Left ())) (((fmap Right) .) <$> const <$> f))
        (Pure (const $ Right 8)))
        (uncurry <$> g)
    ===
    Select (either (const $ Right 8) id <$> Select (Pure (Left ())) (((fmap Right) .) <$> const <$> f))
        (uncurry <$> g)
    ===
    Select (Select (Pure (Left ()))
