...
\noindent
To implement \hs{getEffectsISA}, we apply the natural transformation \hs{toOver}
to the effects of a free selective computation, which recursively
collects the effects that occur in the \hs{Write}'s argument \hs{fv}:

\vspace{0.5mm}
\begin{minted}[xleftmargin=10pt]{haskell}
getEffectsISA :: ISA a -> [RW ()]
getEffectsISA = getOver . runSelect toOver
\end{minted}
\vspace{1mm}
\begin{minted}[xleftmargin=10pt]{haskell}
