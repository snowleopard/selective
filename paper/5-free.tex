...
visualise the effects as a data flow graph, where instructions are shown as
rectangles, data locations as rounded rectangles, and reads/writes as arcs.

% ; we denote registers with green \cmd{reg} label, memory addresses
...
corresponding element of the state monad. To write an effectful value \hs{fv}
...
The free selective construction in combination with the functor \hs{RW} provides
...
\emph{memory-indirect addressing mode}. If we had a \hs{Monad}~\hs{ISA}
instance, we could give the following semantics to the memory-indirect load
instruction \hs{loadMI}:

\vspace{1mm}
\begin{minted}[xleftmargin=10pt]{haskell}
loadMI :: Register -> Address -> ISA Value
loadMI reg addr = read (Mem addr) >>= \x -> write (Reg reg) (read (Mem x))
\end{minted}
\vspace{1mm}

\noindent
Here, we read from a memory cell, then use the monadic bind operator to extract
a value~\hs{x} from the \hs{ISA} type constructor, and use it in a subsequent
memory read. Although this semantics is, in principle, implementable using the
