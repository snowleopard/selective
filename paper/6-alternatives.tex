...
Another alternative that is worth a remark is to simply add the \hs{select}
method to the \hs{Applicative} type class, with the default implementation
...
While beautiful, \hs{biselect} is a bit more awkward to work with than
\hs{select} and is also more subtle when the order in which the arguments are
