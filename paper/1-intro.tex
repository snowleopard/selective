...
(\cmd{http://hackage.haskell.org/package/selective})
...
effects can be used for \emph{conditional execution} of effects, as shown below.
...
an~\hs{s}, which becomes available only \emph{during execution}. We are
...
because the latter would result in wrapping \hs{putStrLn} into an opaque
function, as in \hs{pingPongM}.
...
execution}: in some contexts, both computations can be executed in parallel,
cancelling the second computation if/when the first one evaluates to \hs{Right}.
