...
given \hs{b}. Note, however, that if \hs{a} is a \emph{phantom type
variable}~\cite{leijen2000domain}, i.e. \hs{f}~\hs{a} holds no values of type
\hs{a}, then we can skip the effects in \hs{f}~\hs{(}\hs{a}~\hs{->}~\hs{b)}
unconditionally, e.g. see the selective functor \hs{Under}
in~\S\ref{sec-instances}.
