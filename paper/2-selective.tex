...
reusing\footnote{Fortunately it is possible to reuse \hs{Const} instances
without duplicating any code, by using the new GHC extension
\textsf{DerivingVia}~\citep{blondal2018deriving} and adding
...
effects that will actually occur. This can be seen in the definition of the
instance, which keeps track of effects in both arguments of \hs{select}. The
selective functor \hs{Under}, on the other hand, discards the second argument of
\hs{select} and therefore computes an \emph{under-approximation}, i.e. a list
of effects that are guaranteed to occur.
