...
standard definition of the corresponding type class \hs{Applicative}. The
additional feature that selective functors provide compared to \hs{Applicative}
is the function \hs{select}, which gives meaning to the composition of two
...
\noindent
Think of \hs{select} as a selective function application: you \emph{must apply}
the function of type \hs{a}~\hs{->}~\hs{b} when given a value of type
\hs{Left}~\hs{a}, but you \emph{may skip} the function and associated effects,
and simply return the \hs{b} when given a \hs{Right}~\hs{b}. When convenient, we
will use the infix operator alias \hs{<*?} instead of \hs{select}, which follows
