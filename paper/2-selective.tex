...
\hs{f}~\hs{(}\hs{a}~\hs{->}~\hs{b)} can be skipped unconditionally, e.g. see the
selective functor \hs{Under} in~\S\ref{sec-instances}.}.
...
It is worth emphasising that the relationship between \hs{Applicative} and
\hs{Selective} differs from the relationship between \hs{Applicative} and
\hs{Monad}. Not every \hs{Applicative} is a \hs{Monad}, but every
\hs{Applicative} is a \hs{Selective}, as witnessed by the function \hs{selectA}.
The subclass relationship \hs{Applicative}~\hs{f}~\hs{=>}~\hs{Selective}~\hs{f}
is justified not by possible instances, but by the extra method \hs{select} in
\hs{Selective}. While \hs{select}~\hs{=}~\hs{selectA} is a valid implementation
of \hs{select}, \emph{it is not the only useful implementation}, as will be
demonstrated in~\S\ref{sec-instances}. The applicative-selective-monad hierarchy
therefore reflects method set inclusion:
$\{\hs{<*>}\} \subset \{\hs{<*>}, \hs{select}\} \subset \{\hs{<*>}, \hs{select}, \hs{>>=}\}$.
Table~\ref{tab-operators} compares the three operators in terms their expressive
power. Different applications require different sets of methods. For example, as
we will see in~\S\ref{sec-haxl}, Haxl requires all three: \hs{<*>} gives
parallelism, \hs{select} gives speculative execution, and \hs{>>=} gives
...
Expressive power                          & apply (\hs{<*>}) & select (\hs{<*?}) & bind (\hs{>>=}) \\\hline
