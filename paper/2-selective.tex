...
The \hs{Const}~\hs{m}~\hs{a} functor is an interesting and surprisingly useful
instance of the \hs{Applicative} type class, which stores no values of type
\hs{a}, but keeps track of the applicative structure in the monoid value of type
\hs{m}:
...
records just \hs{"a"} since \hs{apS} is implemented via \hs{select} and
therefore lacks information about the independence of the two effects.
Intuitively, non-rigid selective functors have a richer structure, because
\hs{<*>} is not expressible via \hs{select}.
