...
Having explored various useful combinators that can be implemented using the
selective interface, in this section we look at several basic examples of
selective functors.

As we have already observed at the beginning of this section, any monad can be
given a \hs{Selective} instance simply by using \hs{select}~\hs{=}~\hs{selectM}
as the definition. As an example, below we define a selective instance for
\hs{IO}, and test our \hs{pingPongS} example in an interactive GHC session:
...
@\ghci@ pingPongS = whenS (fmap (=="ping") getLine) (putStrLn "pong")
@\ghci@ pingPongS
hello
@\ghci@ pingPongS
ping
...
\noindent
As desired, the effect \hs{putStrLn}~\hs{"pong"} is conditional.
In~\S\ref{sec-free} we will see how to statically analyse \hs{pingPongS}, i.e.
obtain a list of all possible effects before the execution, which can be done by
interpreting \hs{pingPongS} in the selective functor \hs{Over}, introduced
below.
