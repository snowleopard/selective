...
-- Note: In the current implementation, 'select' costs O(N), where N is the
-- number of effects. It is possible to improve this to O(1) by using the idea
-- developed for free applicative functors by Dave Menendez, see this blog post:
...
instance Functor (Select f) where
    ...
    fmap f (Select g x y) = Select (bimap (f.) f <$> g) x y -- O(1)
...
instance Selective (Select f) where
    select = selectBy (first (flip ($)))
      where
        ...
        selectBy f x (Select g y z) = Select id (selectBy h x y) z -- O(N)
          where
